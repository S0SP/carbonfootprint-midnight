...
import type { State, CarbonCreditPrivateState, Contract, Witnesses } from '../../contract/src/index';

export const carbonCreditPrivateStateKey = 'carbonCreditPrivateState';
export type PrivateStateId = typeof carbonCreditPrivateStateKey;
 ...
 *
 * Since there is only one contract type for the carbon credit tracker example, we only define a
...
export type PrivateStates = {
  /**
   * Key used to provide the private state for {@link CarbonCreditContract} deployments.
   */
  readonly carbonCreditPrivateState: CarbonCreditPrivateState;
};

/**
 * Represents a carbon credit tracker contract and its private state.
 *
 * @public
 */
export type CarbonCreditContract = Contract<CarbonCreditPrivateState, Witnesses<CarbonCreditPrivateState>>;

/**
 * The keys of the circuits exported from {@link CarbonCreditContract}.
 *
 * @public
 */
export type CarbonCreditCircuitKeys = Exclude<keyof CarbonCreditContract['impureCircuits'], number | symbol>;

/**
 * The providers required by {@link CarbonCreditContract}.
 *
 * @public
 */
 ...
 * @public
 */
export type DeployedCarbonCreditContract = FoundContract<CarbonCreditContract>;

/**
 * A type that represents the derived combination of public (or ledger), and private state.
 */
export type CarbonCreditDerivedState = {
   ...
   * the `secretKey` property of {@link CarbonCreditPrivateState} is the secret key of the current user. If
