...
/*
 * This file is the main driver for the Midnight carbon credit tracker example.
...
import { ledger, type Ledger, State } from '../../contract/src/managed/carbon-credit/contract/index.js';
...
import { CarbonCreditPrivateState } from '../../contract/src/witnesses.js';

// @ts-expect-error: It's needed to enable WebSocket usage through apollo
globalThis.WebSocket = WebSocket;

/* **********************************************************************
 * getCarbonCreditLedgerState: a helper that queries the current state of
 * the data on the ledger, for a specific carbon credit tracker contract.
 ...
 * in the carbon credit tracker contract.
 */

export const getCarbonCreditLedgerState = async (
...
You can do one of the following:
  1. Deploy a new carbon credit tracker contract
  2. Join an existing carbon credit tracker contract
  3. Exit
...
const deployOrJoin = async (
  ...
  while (true) {
    ...
    switch (choice) {
      case '1':
        api = await CarbonCreditAPI.deploy(providers, logger);
 ...
 * by the contract to be in the ledger state of the carbon credit tracker.
 */

const displayLedgerState = async (
  providers: CarbonCreditProviders,
  deployedCarbonCreditContract: DeployedCarbonCreditContract,
  logger: Logger,
): Promise<void> => {
  const contractAddress = deployedCarbonCreditContract.deployTxData.public.contractAddress;
  const ledgerState = await getCarbonCreditLedgerState(providers, contractAddress);
  if (ledgerState === null) {
    logger.info(`There is no carbon credit tracker contract deployed at ${contractAddress}`);
  } else {
...
const displayPrivateState = async (providers: CarbonCreditProviders, logger: Logger): Promise<void> => {
  const privateState = await providers.privateStateProvider.get(carbonCreditPrivateStateKey);
  if (privateState === null) {
    logger.info(`There is no existing carbon credit tracker private state`);
  } else {
...
const displayDerivedState = (ledgerState: CarbonCreditDerivedState | undefined, logger: Logger) => {
  if (ledgerState === undefined) {
    logger.info(`No carbon credit tracker state currently available`);
  } else {
    const boardState = ledgerState.state === State.OCCUPIED ? 'occupied' : 'vacant';
...
/* **********************************************************************
 * mainLoop: the main interactive menu of the carbon credit tracker CLI.
...
const mainLoop = async (providers: CarbonCreditProviders, rli: Interface, logger: Logger): Promise<void> => {
  const carbonCreditApi = await deployOrJoin(providers, rli, logger);
  if (carbonCreditApi === null) {
    return;
  }
  let currentState: CarbonCreditDerivedState | undefined;
  const stateObserver = {
    next: (state: CarbonCreditDerivedState) => (currentState = state),
  };
  const subscription = carbonCreditApi.state$.subscribe(stateObserver);
  try {
    while (true) {
      ...
      try {
        switch (choice) {
          case '1': {
            const message = await rli.question(`What message do you want to recordCredit? `);
            await carbonCreditApi.recordCredit(message);
            break;
          }
          case '2':
            await carbonCreditApi.retireCredit();
            break;
          case '3':
            await displayLedgerState(providers, carbonCreditApi.deployedContract, logger);
            break;
          case '4':
...
/* **********************************************************************
 * run: the main entry point that starts the whole carbon credit tracker CLI.
 *
...
export const run = async (config: Config, testEnv: TestEnvironment, logger: Logger): Promise<void> => {
  ...
  try {
    ...
    const zkConfigProvider = new NodeZkConfigProvider<'recordCredit' | 'retireCredit'>(config.zkConfigPath);
    const providers: CarbonCreditProviders = {
      privateStateProvider: levelPrivateStateProvider<PrivateStateId, CarbonCreditPrivateState>({
        privateStateStoreName: config.privateStateStoreName,
        signingKeyStoreName: `${config.privateStateStoreName}-signing-keys`,
        privateStoragePasswordProvider: () => {
          return 'CarbonCredit-Test-2026!';
        },
        accountId: seed,
      }),
