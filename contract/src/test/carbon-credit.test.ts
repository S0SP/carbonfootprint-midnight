...
import { CarbonCreditSimulator } from "./carbon-credit-simulator.js";
import { setNetworkId } from "@midnight-ntwrk/midnight-js-network-id";
import { describe, it, expect } from "vitest";
import { randomBytes } from "./utils.js";
import { State } from "../managed/carbon-credit/contract/index.js";

setNetworkId("undeployed");

describe("CarbonCredit smart contract", () => {
  it("generates initial ledger state deterministically", () => {
    const key = randomBytes(32);
    const simulator0 = new CarbonCreditSimulator(key);
    const simulator1 = new CarbonCreditSimulator(key);
    expect(simulator0.getLedger()).toEqual(simulator1.getLedger());
  });

  it("properly initializes ledger state and private state", () => {
    const key = randomBytes(32);
    const simulator = new CarbonCreditSimulator(key);
  ...
  it("lets you set a message", () => {
    ...
    simulator.recordCredit(message);
  ...
  it("lets you recordCredit another message after taking down the first", () => {
    const simulator = new CarbonCreditSimulator(randomBytes(32));
    const initialPrivateState = simulator.getPrivateState();
    simulator.recordCredit("Life before Death.");
    simulator.retireCredit();
  ...
  it("doesn't let the same user recordCredit twice", () => {
    const simulator = new CarbonCreditSimulator(randomBytes(32));
    simulator.recordCredit(
    ...
    ).toThrow("failed assert: Attempted to recordCredit to an occupied board");
  });

  it("doesn't let different users recordCredit twice", () => {
    const simulator = new CarbonCreditSimulator(randomBytes(32));
    simulator.recordCredit("Ash fell from the sky");
    simulator.switchUser(randomBytes(32));
    expect(() =>
      simulator.recordCredit("I am, unfortunately, the hero of ages."),
    ).toThrow("failed assert: Attempted to recordCredit to an occupied board");
  });
  ...
  it("doesn't let users take down someone elses posts", () => {
    ...
    simulator.switchUser(randomBytes(32));
    expect(() => simulator.retireCredit()).toThrow(
      "failed assert: Attempted to take down recordCredit, but not the current owner",
    );
  });
});
